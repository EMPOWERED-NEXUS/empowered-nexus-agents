...
export const AGENT_MAPPINGS: AgentMapping[] = [
  {
    ...
    detail:
      "Turns aggregate content, quiz, and resource-rights summaries into impact and usage reports.",
    usesFromPackage: [
      "course_count",
      "lesson_count",
      "quiz_attempt_count",
  ...
  },
  {
    ...
    status: "Setup-state",
    detail:
      "Uses course/lesson counts and sample titles to suggest content gaps — no full content export.",
    usesFromPackage: ["content_summary", "course_count", "lesson_count"],
  },
  {
    agent: "Teacher Support Agent",
    role: "Teaching recommendations and classroom readiness",
    status: "Setup-state",
    detail: "Uses readiness flags and quiz aggregates for onboarding guidance — no teacher PII.",
    usesFromPackage: ["readiness_flags", "quiz_summary", "recommended_agent_actions"],
  },
  {
    agent: "Sync and Operations Agent",
    role: "Sync health and Nexus Learn OS handoff",
    status: "Foundation ready",
    detail: "Uses device health and sync log counts — secrets and credentials are never ingested.",
    usesFromPackage: ["device_health_summary", "sync_log_count", "readiness_flags"],
  },
  {
    agent: "ExamShield Agent",
    role: "Assessment integrity and quiz patterns",
    status: "Future",
    detail: "Future phase: aggregate quiz attempt patterns only — no individual attempt detail.",
    usesFromPackage: ["quiz_summary", "quiz_attempt_count", "average_score_percent"],
  },
];
